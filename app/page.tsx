    ...
    <div style={{minHeight:'100vh',background:'var(--bg)',position:'relative',overflow:'hidden'}}>
      <header style={{padding:'24px 40px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid var(--border)',position:'relative',zIndex:2}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div>
            <div style={{fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:18,letterSpacing:'-0.04em',lineHeight:1}}>MC<span style={{color:'var(--accent)'}}>.</span>IQ</div>
            <div style={{fontSize:10,color:'var(--text-faint)',letterSpacing:'0.08em',textTransform:'uppercase',fontFamily:'DM Mono,monospace',marginTop:1}}>Mortgage Call Intelligence</div>
          </div>
        </div>
      ...
      <div style={{padding:'0 40px 80px',maxWidth:960,margin:'0 auto',position:'relative',zIndex:2}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(420px,1fr))',gap:16}}>
          {SCENARIOS.map((s) => (
            <div key={s.id} style={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:20,padding:'28px 28px 24px',display:'flex',flexDirection:'column',gap:20}}>
              ...
              <div style={{display:'flex',gap:10}}>
                <Link href={"/training?scenario="+s.id+"&mode=text"} style={{flex:1,textAlign:'center',padding:'11px 16px',background:'var(--accent)',color:'#020b14',fontWeight:700,fontSize:13,borderRadius:10,textDecoration:'none',fontFamily:'DM Sans,sans-serif'}}>
                  Text Call
                </Link>
                <Link href={"/training?scenario="+s.id+"&mode=voice"} style={{flex:1,textAlign:'center',padding:'11px 16px',background:'transparent',color:'var(--text-muted)',fontWeight:500,fontSize:13,borderRadius:10,textDecoration:'none',border:'1px solid var(--border)',fontFamily:'DM Sans,sans-serif'}}>
                  🎙 Voice Call
                </Link>
              </div>
            </div>
