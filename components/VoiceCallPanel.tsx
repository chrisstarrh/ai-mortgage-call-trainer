...
export function VoiceCallPanel({ scenario, onBack }: Props) {
  ...
  useEffect(() => { return () => { activeRef.current = false; try { recognitionRef.current?.stop(); } catch {} window.speechSynthesis?.cancel(); }; }, []);
  ...
  const speakText = useCallback((text: string): Promise<void> => {
    return new Promise(resolve => {
      ...
      const preferred = voices.find(v => v.lang === 'en-US' && v.name.includes('Female')) || voices.find(v => v.lang === 'en-US') || voices[0];
  ...
  const startListening = useCallback(() => {
    ...
    if (!SR) { setStatusText('Speech not supported — try Chrome'); return; }
    ...
    rec.onend = async () => {
      if (!activeRef.current) return;
      const said = currentSpeechRef.current.trim();
      setCurrentSpeech('');
      if (!said) { startListening(); return; }

      setPhase('thinking');
      setStatusText('Borrower is thinking...');
      const newMsgs = [...messagesRef.current, { role: 'user' as const, content: said }];
      setMessages(newMsgs);
    ...
    rec.onerror = (e: any) => {
      if (activeRef.current && (e.error === 'no-speech' || e.error === 'aborted')) startListening();
    };
        ...
        <>
          <div className="flex items-center gap-3 px-4 py-3 bg-surface border border-border rounded-xl mb-4">
            ...
            <div className="flex-1">
              ...
              {currentSpeech && <p className="text-xs text-accent italic mt-0.5">"{currentSpeech}"</p>}
            </div>
            {phase === 'listening' && (
              <div className="flex items-end gap-0.5 h-6">
                {[3,5,7,5,3,7,4].map((h,i) => (
                  <div key={i} className="w-1 bg-accent rounded-full animate-pulse" style={{height: h*3+'px', animationDelay: i*0.1+'s'}} />
                ))}
              </div>
            )}
          </div>
          <div ref={chatRef} className="flex-1 overflow-y-auto space-y-3" style={{maxHeight:'55vh'}}>
            {messages.filter(m => !m.content.startsWith('[')).map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-sm px-4 py-2.5 rounded-2xl text-sm ${m.role === 'user' ? 'bg-accent text-white' : 'bg-surface border border-border text-primary'}`}>
